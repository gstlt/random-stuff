...
# 01/06/2000
#
#  This Nagios plugin was created to check TIA 6.x status
#
...
print_usage() {
  ...
  echo "  $PROGNAME --batches <ORACLE_SID> <USER> <PASS> <JOB_NAME>"
  echo "  $PROGNAME --help"
...
print_help() {
  ...
  echo "       --->  Requires Oracle user/password, SID and job name"
  echo "       		--->  Requires grant select on dba_scheduler_jobs"
  echo "--help"
...
--batches)
    result=`sqlplus -s ${3}/${4}@${2} << EOF
set pagesize 0;
SELECT UPPER(enabled) FROM dba_scheduler_jobs
where job_name = UPPER('${5}');
EOF`
...
#echo $result
    ...
    if [ -n "`echo $result | grep 'no rows selected'`" ] ; then
		error="UNKNOWN - Batch ${5} does not exist on ${2}."
		echo $error
		exit $STATE_UNKNOWN
    fi

    result=`echo $result | sed -e 's/[0-9]* rows selected.//g' | sed --posix -r 's/([A-Z][0-9]{4})\s([0-9])/\1(Job \2)/g'  | awk '{ print $0 }'`
    
	if [ "$result" = "TRUE" ];	then
		error="OK - Batch ${5} is up on ${2}."
		echo $error
		exit $STATE_OK
	fi

    echo "CRITICAL - Batch ${5} is DOWN on ${2}"
    exit $STATE_CRITICAL
    ;;
*)
    print_usage
